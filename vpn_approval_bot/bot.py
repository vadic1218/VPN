...
def admin_row_to_json(row: dict[str, Any], last_seen: dict[str, str] | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "source": "сайт" if str(row.get("source") or "").startswith("web") else "Telegram",
        "chat_id": int(row.get("chat_id") or 0),
...
<style>
*{{box-sizing:border-box}}html,body{{overflow-x:hidden}}body{{margin:0;min-height:100vh;background:#101823;color:#f8fafc;font-family:Inter,Segoe UI,Arial,sans-serif}}.wrap{{width:min(1280px,100%);margin:auto;padding:28px 18px 42px}}header{{display:flex;align-items:center;justify-content:space-between;gap:14px;margin-bottom:22px}}h1{{font-size:clamp(30px,6vw,58px);line-height:1;margin:0}}.status{{padding:9px 12px;border-radius:8px;background:#223044;color:#bad3ef}}.grid{{display:grid;grid-template-columns:minmax(320px,.8fr) minmax(0,1.2fr);gap:18px}}section{{min-width:0;padding:20px;border:1px solid #2c3b52;background:#172231;border-radius:8px}}label{{display:block;margin:12px 0 7px;color:#bdd1e8}}input,select{{width:100%;border:1px solid #3c5270;background:#0f1722;color:#fff;border-radius:8px;padding:13px;font:inherit}}.check{{display:flex;align-items:center;gap:10px;margin-top:12px;color:#bdd1e8}}.check input{{width:auto}}button,a.btn{{display:inline-flex;align-items:center;justify-content:center;min-height:46px;border:0;border-radius:8px;padding:12px 16px;font:inherit;font-weight:700;background:#6d77ff;color:#fff;text-decoration:none;cursor:pointer}}button.secondary,a.secondary{{background:#25354b}}button:disabled{{opacity:.55;cursor:not-allowed}}.actions{{display:flex;flex-wrap:wrap;gap:10px;margin-top:16px}}.muted{{color:#91a8c2;line-height:1.5}}.result{{overflow:auto;overflow-wrap:anywhere;word-break:break-word;white-space:normal;background:#0c131d;border:1px solid #26364c;padding:14px;border-radius:8px;margin-top:14px}}pre{{margin:0;white-space:pre-wrap;overflow-wrap:anywhere;word-break:break-word}}.hide{{display:none}}@media(max-width:900px){{.grid{{grid-template-columns:1fr}}header{{align-items:flex-start;flex-direction:column}}}}
</style>
</head>
<body>
<main class="wrap">
    ...
    class RoutingHandler(BaseHTTPRequestHandler):
        ...
        def _handle_public_post(self, path: str) -> None:
            ...
            try:
                ...
                token_row = store.get_request_by_web_token(web_token)
                if token_row and not (contact_keys(contact) & Store._request_contact_keys(token_row)):
                    web_token = uuid.uuid4().hex
