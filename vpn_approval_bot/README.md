...
Users can open `Моя подписка` to see their current status, price list, and action buttons for reissue, plan changes, and Happ routing. The routing action sends only a button for the Happ routing link; if Telegram blocks the direct `happ://` button, the bot falls back to a copy button without printing the full link in chat. TikTok domains go through VPN, while Russian IPs, VK, Yandex, Gosuslugi, Russian media, marketplaces, job sites, common Russian banks, and browser captcha services go direct outside the VPN. This is client-side split tunneling, so it must be applied in the user's VPN app.
